...
already-trained frozen codec (VjepaModel + MyBottleneck only -- no decoder, no backward pass).
Writes {"latent_mean": float, "latent_std": float, "n_elements": int} as JSON to --output.

Full fork of compute_latent_stats.py for the V-JEPA track (VjepaModel in place of DinoModel), same
reasoning as train_codec_vjepa.py/evaluate_codec_vjepa.py: kept as a separate script on purpose,
not a shared core loop, so each track's logic reads on its own.

Deliberately not an online EMA retrofitted into train_codec_vjepa.py (see notes/deviations.md) --
a direct pass over a couple hundred batches is simpler, needs no changes to the already-run codec
training, and works immediately on any already-trained checkpoint.

Two data modes, same convention as train_codec_vjepa.py:
  ...
  - Default: one fixed synthetic video reused every batch -- a mechanism check, not a real
    statistic (matches train_codec_vjepa.py's own framing for its no-index-path mode).

--codec-checkpoint is optional (default None -> random-init bottleneck) so the mechanism/wiring
can be checked without a real checkpoint on hand; pass a real one for a real statistic. No real
V-JEPA codec checkpoint exists yet (see notes/vjepa_next_session.md) -- this script is verified
against the random-init/synthetic path only, ready the moment a real one does.

Deliberately CPU-friendly, unlike train_codec_vjepa.py (no backward pass, no GradScaler needed) --
...
def _keep_convolutions_in_bf16(module: torch.nn.Module) -> None:
    ...
    -- same helper as scripts/compute_latent_stats.py and mini_mira.world_model.latent_world_model
...
def parse_args() -> argparse.Namespace:
    ...
    # Left at compute_latent_stats.py's own default, unchanged -- matches train_codec_vjepa.py's
    # own precedent (a CPU-friendly fallback only; a real run always passes --config explicitly).
    ...
    parser.add_argument("--output", default="latent_stats_vjepa.json")
...
def build_next_video(args: argparse.Namespace):
    ...
    same two-mode shape as train_codec_vjepa.py's build_next_video."""
    if args.index_path:
