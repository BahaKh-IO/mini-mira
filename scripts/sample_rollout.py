"""Standalone rollout sampling + eval for an already-trained DINO-track world-model checkpoint --
no training loop, no optimizer, just: load the real checkpoint, roll out on real held-out clips
with a small context (a genuinely LONG generated region), save the rollout videos, compute the
same Frechet DINO/Inception Distance + PSNR/LPIPS/SSIM + drift metrics train_world_model.py's own
periodic full-eval computes, print + save the numbers to JSON.

Built for a genuinely comparable DINO-vs-V-JEPA "same long rollout" benchmark -- see
scripts/sample_rollout_vjepa.py for the V-JEPA-track sibling (full duplicate, same reasoning as
every other DINO/V-JEPA script pair in this project). Pass the SAME --seed on both tracks'
invocations to pull the same real held-out clips (before each track's own resize) for a genuinely
apples-to-apples comparison, not just "both evaluated on some held-out data."
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/scaled_300m.yaml")
    parser.add_argument("--codec-checkpoint", required=True)
    parser.add_argument("--wm-checkpoint", required=True, help="Trained world-model checkpoint (checkpoint.pth)")
...
def main() -> None:
    ...
    # dino=None (default) -- LatentWorldModel builds its own DinoModel internally, same as
    # train_world_model.py's own real construction (no injection needed for the DINO track).
