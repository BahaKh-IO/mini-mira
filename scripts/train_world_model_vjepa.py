...
def main() -> None:
    ...
    for step in range(start_step, args.steps):
        ...
        # per loss term per micro-step (this loop's original behavior, inherited unchanged from
        # train_world_model.py) serializes what should be an async pipeline. Identical root cause,
        # identical fix already proven for real in train_codec_vjepa.py this project (found via
        # nvidia-smi dmon showing the GPU alternating 0%/100%) -- ported here after the same
        # pattern showed up live on this exact script. Converted to floats once, after the loop,
        # instead -- same final numbers, far fewer sync points (was num_loss_terms *
